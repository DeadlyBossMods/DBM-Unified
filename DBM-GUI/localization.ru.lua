...
L.BadTimerAlert				= "Показывать сообщение чата, когда DBM обнаруживает неисправный таймер с ошибкой не менее 1 секунды"
L.BadIDAlert				= "Показывать сообщение чата, когда DBM обнаруживает недействительный идентификатор заклинания или журнала."
L.ShowReminders				= "Показывать напоминания об отсутствующих подмодах, отключенных подмодах, исправлениях подмодов, устаревших подмодах и включенном беззвучном режиме"
...
L.EnableWBSharing 			= "Поделитесь, когда Вы пулите / побеждаете мирового босса со своей гильдией и друзьями из Battle.net, которые находятся в одном игровом мире."
L.SpamBlockNoSpecWarnFlash			= "Не показывать специальное объявление экранной вспышки"
L.SpamBlockNoSpecWarnVibrate		= "Не вибрируйте контроллер по специальному объявлению"
...
L.UseNameplateHandoff		= "Передайте запросы ауры нэймплейтов поддерживаемым аддонам (KuiNameplates, Threat Plates, Plater) вместо внутренней обработки. Это рекомендуемый вариант, так как он позволяет выполнять более продвинутые функции и настройку с помощью надстройки нэймплейтов."
