...
L.WorldBossNearAlert		= "Проигрывать звук проверки готовности и мигать иконкой приложения, когда неподалеку пуллят нужных Вам боссов вне подземелья (перезаписывает настройку для конкретного босса)"
...
L.BarSkin					= "Bar skin"--I need to check it, then I’ll translate it
...
L.FilterTankSpec			= "Фильтровать уведомления для роли танка, когда не в соответствующей специализации (то есть не в 'Танке')<br/>(Примечание: отключение этого параметра не рекомендуется для большинства пользователей, поскольку теперь все уведомления 'Таунта' включены по умолчанию)"
L.FilterDispels				= "Фильтровать уведомления для рассеиваемых заклинаний, если Ваш диспел находится в кулдауне"
--L.FilterCrowdControl				= "Filter announcements for crowd control based interrupts if your CC is on cooldown"
L.FilterTrashWarnings		= "Фильтровать уведомления для трэша в подземельях с соратниками, а также обычных, героических и тривиальных (превышенных по уровню) подземельях"
