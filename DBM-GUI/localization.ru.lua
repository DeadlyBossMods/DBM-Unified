...
L.ShowWAKeys						= "Показывать ключи WeakAuras рядом с именами заклинаний, чтобы помочь в написании WeakAuras с использованием триггеров Boss Mod (Не будут отображаться ключи для опций, в которых используется несоответствующий ключ из другого заклинания, например, пользовательские отключенные локализованные оповещения, не привязанные к заклинанию или идентификатору журнала)"
-- Missing phrases. End ..
