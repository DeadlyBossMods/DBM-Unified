...
L.SpamSpecInformationalOnly			= "Удалить все обучающие текстовые/речевые оповещения из специальных объявлений (требуется перезагрузка пользовательского интерфейса). Оповещения по-прежнему будут отображаться и воспроизводить звук, но они будут общими и недирективными."
