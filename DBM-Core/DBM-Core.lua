...
if isRetail then
	DBM.DisplayVersion = "10.1.13 alpha"
...
function bossModPrototype:SetStage(stage)
	...
	if self.inCombat then--Safety, in event mod manages to run any phase change calls out of combat/during a wipe we'll just safely ignore it
		...
		SendBWMessage("BigWigs_SetStage", self, GetBWStage(self))
...
do
	...
	function announcePrototype:Show(...) -- todo: reduce amount of unneeded strings
		if not self.option or self.mod.Options[self.option] then
			...
			SendBWMessage("BigWigs_Message", self.mod, GetBWKey(self), message, nil, self.icon)
...
--		["addscustom"] = "spell",
	...
	function specialWarningPrototype:Show(...)
		...
		if (not self.option or self.mod.Options[self.option]) and not moving and frame then
			...
			SendBWMessage("BigWigs_Message", self.mod, GetBWKey(self), message, nil, self.icon) -- send unembellished message rather than icon/color/note augmented text
...
do
	...
	function timerPrototype:Start(timer, ...)
		...
		if not self.option or self.mod.Options[self.option] then
			...
			SendBWMessage("BigWigs_StartBar", self.mod, GetBWKey(self), msg, timer, self.icon, msg:sub(1, 1) == "~", timer)
			...
			if not tContains(self.startedTimers, id) then--Make sure timer doesn't exist already before adding it
				...
				self.startedTexts[id] = msg--BW callbacks need text as unique key. We'd need to schedule cleanup in half a dozen places, but stale entries are harmless so just don't bother until no-args :Stop().
			end
	...
	function timerPrototype:Stop(...)
		if select("#", ...) == 0 then
			...
			twipe(self.startedTexts)--All timers stopped, so we know all entries are stale.
		else
			local id = self.id..pformat((("\t%s"):rep(select("#", ...))), ...)
			for i = #self.startedTimers, 1, -1 do
	...
	function timerPrototype:Update(elapsed, totalTime, ...)
		...
		SendBWMessage("BigWigs_StartBar", self.mod, GetBWKey(self), self.startedTexts[id], newRemaining, self.icon, self.startedTexts[id]:sub(1, 1) == "~", totalTime)
	...
	function timerPrototype:AddTime(extendAmount, ...)
		...
		if not bar then
			...
			local elapsed, totalTime = (bar.totalTime - bar.timer), bar.totalTime
			if elapsed and totalTime then
				totalTime = totalTime + extendAmount
				local newRemaining = totalTime - elapsed
	...
	function timerPrototype:RemoveTime(reduceAmount, ...)
		...
		if not bar then
			...
			local elapsed, totalTime = (bar.totalTime - bar.timer), bar.totalTime
			if elapsed and totalTime then
				totalTime = totalTime - reduceAmount
				local newRemaining = totalTime - elapsed
		...
		local obj = setmetatable(
			{
				...
				startedTimers = {},
				startedTexts = {},--Needed for BW messages
				mod = self,
			},
			mt
		)
